\HeaderA{panelplot}{Panel Plot}{panelplot}
\keyword{models}{panelplot}
...
\begin{Arguments}
\begin{ldescription}
\item[\code{data}] A list consisting of elements, each of which
consists of x, y, xlim and ylim vectors
\item[\code{panel}] The panel function to be plotted
\item[\code{totrows}] The number of rows in the plot layout
\item[\code{totcols}] The number of columns in the plot layout
\item[\code{oma}] Outer margin area
\item[\code{par.strip.text}] A data frame with column cex
\end{ldescription}
\end{Arguments}
\begin{Author}\relax
J.H. Maindonald
